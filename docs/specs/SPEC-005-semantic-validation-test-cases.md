...
| SV-136g | A claimed response completes after the proxy's monotonic trigger deadline, including while healing; or the harness observes an on-time completion after its own polling deadline. | Only the proxy's explicit on-time completion evidence counts. Late drops remain reported; polling latency cannot extend the budget or invalidate an already completed drop. |
| SV-136h | A matching EndTxn response contains a broker error, followed by a successful retry. | Forward the error, release its reserved occurrence, and drop the successful response. An error response cannot satisfy the commit-loss proof. |
| SV-136i | A dropped EndTxn message is retried by the client after its fault healed. | After the process fence, the report shows the retry witness (time, delay after the drop, client ID) on that dropped message; an unreadable event file adds `network-fault.retry-evidence-unavailable` and changes no verdict. |
...
| SV-183e | A pinned expected failure occurs with an ineffective kill, missed network fault, missing or foreign subject origins, failed phases, or incomplete fence/oracle evidence. | Retain the attempt's data failure; the scenario verdict is inconclusive and the expectation does not match (SPEC-001 R8.7a). |
...
| SV-183g | A configured incarnation's class log is missing while its replacement has valid loads, or classes appear only across different TaskManager incarnations. | Origins remain unconfirmed. Inventory comes from configured incarnations, and one TaskManager must contain every entry class. Failed-startup retries retain distinct log paths. |
