...
- **E4.4** Expected `fail` matches only an attempt result of `fail` from the
  ...
  (SPEC-001 R8.7a). A matching data failure with unconfirmed subject or fault
  evidence, or incomplete phase/fence/oracle evidence, remains an attempt failure
  but gives an `inconclusive` scenario verdict.
