...
**Resolves:** decision D1 in the implementation roadmap (local working document).
...
- Not a replacement for the implementation plan (local working document), which
  holds the architecture and the scenario catalogue.
...
> **Note — reversal of an earlier recommendation.** The roadmap originally
> recommended deferring matrix/parameterization out of v1. That was wrong given
> §3: differential experiments *are* parameterization, so the mechanism is core
> rather than a convenience. What stays out of v1 is the automatic *expansion* of
...
- **R3.9** There is **one** differential mechanism. Roadmap step 4.4
  ("baseline differential") is not a separate feature; it is this mechanism with
