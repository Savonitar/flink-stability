...
public final class ConnectorClusterBundleBuilder {
    ...
    /**
     * Precomputes every declared connector lock and every distinct target binding for one side.
     * The returned list follows first target-image encounter order and is immutable.
     */
    public List<PreparedConnectorBundle> buildAll(
            PreparedScenarioPlan plan,
            ScenarioSide side) {
        Objects.requireNonNull(plan, "plan");
        Objects.requireNonNull(side, "side");
        List<ConnectorClosureLock> allLocks = lockFactory.createAll(plan, side);
        List<PreparedConnectorBundle> bundles = new ArrayList<>();
        for (String target : lockFactory.targetFlinkImageReferences(plan, side)) {
            bundles.add(build(plan, side, target, allLocks));
        }
    ...
    /**
     * Builds from precomputed locks. Locks for other sides, aliases, or target images are
     * ignored; every job-referenced alias must have exactly one matching lock.
     */
    public PreparedConnectorBundle build(
            PreparedScenarioPlan plan,
            ScenarioSide side,
            String targetFlinkImageReference,
            List<ConnectorClosureLock> availableLocks) {
        Objects.requireNonNull(plan, "plan");
        Objects.requireNonNull(side, "side");
        requireNonBlank(targetFlinkImageReference, "targetFlinkImageReference");
        List<ConnectorClosureLock> locks = List.copyOf(Objects.requireNonNull(
        ...
        List<ConnectorClosureLock> selectedLocks = new ArrayList<>(referencedAliases.size());
        for (Map.Entry<String, String> reference : referencedAliases.entrySet()) {
            List<ConnectorClosureLock> matches = locks.stream()
                    .filter(lock -> lock.side() == side
                            && lock.alias().equals(reference.getKey())
                            && lock.targetFlinkImageReference().equals(
                                    targetFlinkImageReference))
                    .toList();
            if (matches.size() != 1) {
                String qualifier = matches.isEmpty() ? "No" : "More than one";
                issues.add(issue(
                        plan,
                        side,
                        "artifact.connector.bundle-missing-closure",
                        reference.getValue(),
                        qualifier + " target-specific connector lock exists for alias '"
                                + reference.getKey() + "' and image '"
                                + targetFlinkImageReference + "'"));
                continue;
            }
            ConnectorClosureLock lock = matches.getFirst();
            PreparedConnectorClosure closure = plan.connectorClosure(side, reference.getKey())
                    .orElse(null);
            if (closure == null || !describes(lock, closure)) {
                issues.add(issue(
                        plan,
                        side,
                        "artifact.connector.bundle-missing-closure",
                        reference.getValue(),
                        "The target-specific lock for alias '" + reference.getKey()
                                + "' does not describe this prepared connector closure"));
                continue;
            }
    ...
    private static boolean describes(
            ConnectorClosureLock lock,
            PreparedConnectorClosure closure) {
        if (!lock.alias().equals(closure.alias())
                || !lock.connectorPath().equals(closure.connectorPath())
                || lock.dependencyMode() != closure.dependencyMode()
                || lock.classpath().size() != closure.classpath().size()
                || !lock.descriptors().equals(ConnectorClosureLockFactory.sortedDescriptors(
                        closure.consultedPoms()))
                || !lock.mediationDecisions().equals(closure.conflicts())) {
            return false;
        }
        for (int index = 0; index < lock.classpath().size(); index++) {
            if (lock.classpath().get(index).artifact() != closure.classpath().get(index)) {
                return false;
            }
        }
        return true;
